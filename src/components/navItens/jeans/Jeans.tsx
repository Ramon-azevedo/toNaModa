...
function Jeans() {
    ...
    const [jeans, setJeans] = useState<Jeans01[]>([]);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        axios
            .get("http://localhost:8080/jeans/getJeans")
            .then((response) => {
                setJeans(response.data);
            })
            .catch((err) => {
                setError("Erro ao carregar a moda jeans.");
                console.error(err);
            })
    },[]);
                ...
                <div>
                    ...
                    <div className="subItens">
                        {error ? (
                            ...
                            jeans.map((jea) => (
                            <ul key={jea.id} className="itens">
                                <li><img src={jea.imagem} alt={jea.nome} className="img" /></li>
                                <li>{jea.nome}</li>
                                <li>{jea.preco}</li>
                                <li>{jea.vezesCartao}</li>
                            </ul>
                            ))
                        )}
                    </div>
