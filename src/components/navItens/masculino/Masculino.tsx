...
function Masculino() {
    ...
    const [masculino, setMasculino] = useState<Masculino01[]>([]);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        axios
            .get("http://localhost:8080/masculino/getMasculino")
            .then((response) => {
                setMasculino(response.data);
            })
            .catch((err) => {
                setError("Erro ao carregar a moda masculina.");
                console.error(err);
            })
    },[]);
                ...
                <div>
                    ...
                    <div className="subItens">
                        {error ? (
                            ...
                            masculino.map((masc) => (
                            <ul key={masc.id} className="itens">
                                <li><img src={masc.imagem} alt={masc.nome} className="img" /></li>
                                <li>{masc.nome}</li>
                                <li>{masc.preco}</li>
                                <li>{masc.vezesCartao}</li>
                            </ul>
                            ))
                        )}
                    </div>
