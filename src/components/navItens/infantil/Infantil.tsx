...
function Infantil() {
    ...
    const [infantil, setInfantil] = useState<Infantil01[]>([]);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        axios
            .get("http://localhost:8080/infantil/getInfantil")
            .then((response) => {
                setInfantil(response.data);
            })
            .catch((err) => {
                setError("Erro ao carregar a moda infantil.");
                console.error(err);
            })
    },[]);
                ...
                <div>
                    ...
                    <div className="subItens">
                        {error ? (
                            ...
                            infantil.map((inf) => (
                            <ul key={inf.id} className="itens">
                                <li><img src={inf.imagem} alt={inf.nome} className="img" /></li>
                                <li>{inf.nome}</li>
                                <li>{inf.preco}</li>
                                <li>{inf.vezesCartao}</li>
                            </ul>
                            ))
                        )}
                    </div>
