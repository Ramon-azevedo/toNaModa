...
function Casa() {
    ...
    const [casa, setCasa] = useState<Casa01[]>([]);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        axios
            .get("http://localhost:8080/casa/getCasa")
            .then((response) => {
                setCasa(response.data);
            })
            .catch((err) => {
                setError("Erro ao carregar a moda casa.");
                console.error(err);
            })
    },[]);
                ...
                <div>
                    ...
                    <div className="subItens">
                        {error ? (
                            ...
                            casa.map((cas) => (
                            <ul key={cas.id} className="itens">
                                <li><img src={cas.imagem} alt={cas.nome} className="img" /></li>
                                <li>{cas.nome}</li>
                                <li>{cas.preco}</li>
                                <li>{cas.vezesCartao}</li>
                            </ul>
                            ))
                        )}
                    </div>
